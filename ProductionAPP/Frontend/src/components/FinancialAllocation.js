  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 flex flex-col justify-between gap-5">
          <div className="space-y-1">
            <h3 className="text-sm font-bold text-white uppercase tracking-wider">Profit Maximizer</h3>
            <p className="text-[10px] text-zinc-500">Projected weekly financial returns based on current demand forecasts.</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-800 flex justify-between items-center">
              <div>
                <span className="block text-[10px] text-zinc-500 uppercase font-semibold">Weekly ROI Estimate</span>
                <span className="text-2xl font-black text-white">+{roi}%</span>
              </div>
              <span className="p-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold flex items-center gap-1">
            ...
            <div className="space-y-2.5 text-xs">
              <div className="flex justify-between border-b border-zinc-850 pb-2">
                <span className="text-zinc-400">Projected Demand Intake</span>
                <span className="font-semibold text-white">₹{weeklySales.toLocaleString()}</span>
              </div>
              <div className="flex justify-between border-b border-zinc-850 pb-2">
                <span className="text-zinc-400">Procurement Cost Basis</span>
                <span className="font-semibold text-zinc-350">₹{weeklyCosts.toLocaleString()}</span>
              </div>
              <div className="flex justify-between border-b border-zinc-850 pb-2">
                <span className="text-zinc-400">Projected Weekly Profit</span>
                <span className="font-semibold text-emerald-400">₹{marginAmt.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-zinc-400">Average Net margin</span>
        ...
        <div className="lg:col-span-2 bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 flex flex-col gap-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-800 pb-3">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Capital Allocation efficiency</h3>
          ...
          <div className="space-y-4">
            {categoriesAlloc.map(cat => {
              ...
              return (
                <div key={cat.name} className="p-3.5 rounded-xl bg-zinc-950 border border-zinc-850/80 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                  ...
                  <div className="md:col-span-2 space-y-1">
                    <div className="flex justify-between items-center text-[10px]">
                      <span className="text-zinc-500">Allocated: <span className="font-semibold text-zinc-300">₹{cat.allocated.toLocaleString()}</span></span>
                      <span className="text-zinc-500">Demand: <span className="font-semibold text-zinc-300">₹{cat.demandVal.toLocaleString()}</span></span>
                    </div>
                  ...
                  <div className="md:col-span-1 text-right text-xs">
                    {isLeak ? (
                      <div className="space-y-0.5">
                        <span className="text-rose-400 font-bold block">- ₹{surplusVal.toLocaleString()}</span>
          ...
          <div className="p-3.5 rounded-lg bg-rose-950/15 border border-rose-900/30 text-[10px] text-zinc-400 leading-relaxed flex items-start gap-2">
            <TrendingDown className="w-4 h-4 text-rose-500 shrink-0 mt-0.5" />
            <div>
              <span className="font-semibold text-rose-400 block uppercase tracking-wider mb-0.5">Over-allocation Risk Alert:</span>
