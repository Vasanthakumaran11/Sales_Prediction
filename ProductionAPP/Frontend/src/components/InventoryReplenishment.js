...
export default function InventoryReplenishment({ storeInfo, isMultiStoreMode }) {
  ...
  // Bulk Consolidation calculation details
  ...
  const bulkDiscount = 0.15; // 15% discount for consolidated orders
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-zinc-800 pb-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-white placeholder-zinc-650 focus:outline-none focus:border-emerald-500 w-44"
          />
        </div>
      </div>
      ...
      {showBulkConsolidator && (
        <div className="p-6 rounded-2xl bg-zinc-900/60 border border-blue-500/30 backdrop-blur-xl relative overflow-hidden space-y-5">
          <div className="absolute top-0 right-0 w-48 h-48 bg-blue-500/5 rounded-full blur-3xl pointer-events-none" />
          
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-500/10 border border-blue-500/25 text-[10px] text-blue-400 font-bold uppercase tracking-wider mb-2">
                <Layers className="w-3 h-3" /> Multi-Store Purchasing Engine
              </div>
              <h3 className="text-lg font-bold text-white">Consolidated ROP Order Planner</h3>
              <p className="text-xs text-zinc-400 mt-0.5">
            ...
            {!consolidatedOrderPlaced && (
              <div className="text-right">
                <span className="block text-[10px] text-zinc-500 uppercase font-semibold">Consolidated Savings</span>
                <span className="text-lg font-bold text-emerald-400">15% Volume Discount Active</span>
              </div>
            )}
          </div>
          ...
          {!consolidatedOrderPlaced ? (
            <div className="space-y-4">
              {/* Aggregation Table */}
              <div className="overflow-x-auto border border-zinc-800/80 rounded-xl bg-zinc-950/80">
                <table className="w-full text-left text-xs">
                  <thead>
                    <tr className="bg-zinc-900 border-b border-zinc-800 text-zinc-400 font-semibold text-[9px] uppercase tracking-wider">
                      ...
                      <th className="p-3 text-right font-bold text-zinc-200">Unified Qty</th>
                  ...
                  <tbody className="divide-y divide-zinc-900">
                    {bulkItems.map(item => {
                      ...
                      return (
                        <tr key={item.name} className="hover:bg-zinc-900/20 text-zinc-300">
                          <td className="p-3 font-semibold text-white">{item.name}</td>
                          <td className="p-3 text-right text-zinc-400">{item.balaji} {item.unit}</td>
                          <td className="p-3 text-right text-zinc-400">{item.shiva} {item.unit}</td>
                          <td className="p-3 text-right text-zinc-400">{item.surya} {item.unit}</td>
                          <td className="p-3 text-right font-bold text-emerald-400">{totalQty} {item.unit}</td>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 rounded-xl bg-zinc-950 border border-zinc-800">
                <div className="space-y-1">
                  <div className="text-xs text-zinc-400">
                    Standard Cost: <span className="line-through text-zinc-600">₹{totalStandardCost.toLocaleString()}</span>
                  </div>
                  <div className="text-sm">
                    Consolidated Contract Price: <span className="font-bold text-white">₹{totalDiscountedCost.toLocaleString()}</span>{' '}
                    <span className="text-emerald-400 text-xs font-semibold">(Saved ₹{(totalStandardCost - totalDiscountedCost).toLocaleString()})</span>
                  </div>
                </div>
                <button
            ...
            <div className="p-8 text-center bg-zinc-950/80 rounded-xl border border-emerald-500/20 py-10 space-y-4">
              <CheckCircle className="w-12 h-12 text-emerald-500 mx-auto" />
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-white">Consolidated Purchase Order Dispatched</h4>
      ...
      {criticalItems.length > 0 && !showBulkConsolidator && (
        <div className="bg-rose-950/15 border border-rose-900/30 rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xs font-bold text-rose-400 uppercase tracking-wider">ROP Critical Deficit Alerts</h3>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {criticalItems.slice(0, 4).map(item => (
              <div key={item.name} className="p-3 rounded-lg bg-zinc-950 border border-rose-900/30 flex justify-between items-center text-xs">
                <div>
                  <span className="font-bold text-white block">{item.name}</span>
                  <span className="text-[10px] text-zinc-500">Category: {item.category}</span>
                </div>
                <div className="text-right">
                  <span className="font-bold text-rose-400 block">{item.stock} Units left</span>
                  <span className="text-[9px] text-zinc-500 font-semibold">ROP: {item.rop}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5 space-y-4">
        <div>
          <h3 className="text-sm font-bold text-white uppercase tracking-wider">Inventory Metrics Grid (Layer 6)</h3>
          ...
          <table className="w-full border-collapse text-left text-xs">
            ...
            <tbody className="divide-y divide-zinc-900">
              {filteredItems.map(item => {
                ...
                return (
                  <tr key={item.name} className="hover:bg-zinc-900/40 text-zinc-300 transition-colors">
                    <td className="p-3">
                      <div>
                        <span className="font-bold text-white block">{item.name}</span>
                    ...
                    <td className="p-3 text-right font-medium text-zinc-400">{item.minStock}</td>
                    <td className="p-3 text-right font-medium text-zinc-400">{item.rop}</td>
                    <td className="p-3 text-right font-bold text-emerald-400">{item.eoq}</td>
        ...
        <div className="p-3 rounded-lg bg-zinc-950 border border-zinc-850 text-[10px] text-zinc-500 flex items-start gap-2 leading-relaxed">
          <Info className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
          <div>
            <span className="font-semibold text-zinc-400 block">Safety stock and ROP calculations details:</span>
            <p>
